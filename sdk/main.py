...
if __name__ == '__main__':
    ...
    Update = contract_factory.getUpdate()
    ...
    m.start()
    #print(m.weights)
    weights = pickle.dumps(m.weights)
    w_id = fdfs.up(weights)

    
    update_sum = (np.array(m.weights)-np.array(m.weights)).tolist()
    update_sum = pickle.dumps(update_sum)
    us_id = fdfs.up(update_sum)
    us_id = pickle.dumps(us_id)
    #init update_sum
    Update.clean(us_id)
    Gmodel.set(pickle.dumps(w_id))
