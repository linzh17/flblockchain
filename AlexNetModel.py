...
C1 = AlexNetModel(train_generator,size,BATCH_SIZE,EPOCHS)
C1.start()
weights = C1.weights
...
ret = client.upload_by_buffer(weights)
#file_id = ret['Remote file_id'].decode('ascii')
file_id = ret['Remote file_id']
print(file_id)
buffer = client.download_to_buffer(file_id)
#print(buffer)
weights = pickle.loads(buffer['Content'])
#print(weights)
