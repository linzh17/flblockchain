...
from sdk.contractFactory import ContractFactory
...
def start_client(i):
    ...
    model = AlexNetModel(train_generator,size,BATCH_SIZE,EPOCHS)
    client = Client(i,model,contract_factory)
    client.start()


if __name__ == '__main__':
    Gmodel = contract_factory.getGmodel()
    m_gen = gen.flow(X_train[0:12000], y_train[0:12000], batch_size=BATCH_SIZE)
    m = AlexNetModel(m_gen,12000,BATCH_SIZE,EPOCHS)
    m.start()
    ...
    for i in range(1,6):
        ...
        p = multiprocessing.Process(target=start_client(i))
        jobs.append(p)
        p.start()
    # TODO: randomly kill
